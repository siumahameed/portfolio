...
const components: Components = {
  code({ className, children, ...props }) {
    ...
    return (
      <code
        className="rounded-md bg-[var(--bg-secondary)] px-1.5 py-0.5 text-sm text-[var(--accent)] font-mono"
        {...props}
      >
        {children}
      </code>
  ...
  pre({ children }) {
    return <div className="my-6 overflow-x-auto rounded-xl border border-[var(--border)]">{children}</div>;
  },
  h2({ children }) {
    return <h2 className="mt-10 mb-4 text-xl font-bold text-[var(--text-primary)] font-heading">{children}</h2>;
  },
  h3({ children }) {
    return <h3 className="mt-8 mb-3 text-lg font-semibold text-[var(--text-primary)] font-heading">{children}</h3>;
  },
  p({ children }) {
  ...
  blockquote({ children }) {
    return (
      <blockquote className="my-6 border-l-[3px] border-[var(--accent)] bg-[var(--accent-subtle)] py-3 pl-5 pr-4 italic text-[var(--text-secondary)] rounded-r-xl">
        {children}
      </blockquote>
    );
  },
  ...
  return (
    <article>
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-[var(--text-primary)] sm:text-4xl font-heading">
          {article.title}
        </h1>
